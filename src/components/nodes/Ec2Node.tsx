  ...
  return (
    ...
    >
      ...
      {data.metrics?.instanceType && (
        <div className="mt-2 text-[12px] font-normal text-[var(--gl-text-muted)] flex justify-between">
          <span>Type:</span>
          <span>{data.metrics.instanceType}</span>
        </div>
      )}
      {data.metrics?.state && (
        <div className="mt-1 flex items-center gap-2 text-[12px] font-normal text-[var(--gl-text-muted)] bg-indigo-50/50 dark:bg-indigo-950/50 p-2 rounded-md border border-indigo-100 dark:border-indigo-900/50">
          <span className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: data.metrics.state === 'running' ? '#1D9E75' : '#BA7517' }}></span>
          {data.metrics.state.toUpperCase()}
        </div>
      )}
      {data.insights && (
        <div className="mt-2 text-[12px] font-normal text-[var(--gl-text-muted)] bg-slate-100/50 dark:bg-slate-950/50 p-2 rounded-md border border-slate-200/50 dark:border-slate-800/50">
          {data.insights}
        </div>
      )}
    </motion.div>
