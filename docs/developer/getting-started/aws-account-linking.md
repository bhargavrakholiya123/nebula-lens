...
- Knowledge of the tenant-specific External ID provided by GravityLens during the onboarding process.
...
- **Required Permissions**: Read-only access to specific AWS services. Users can either attach the AWS-managed `ReadOnlyAccess` policy or use a scoped-down inline permission policy.
...
- **Condition (`sts:ExternalId`)**: Crucial for security. It prevents the **Confused Deputy Problem** by ensuring that GravityLens only assumes this role when explicitly acting on behalf of the specific tenant who owns the `unique-external-id-per-tenant`.
...
- **Confused Deputy Protection**: The mandated `ExternalId` ensures that one GravityLens customer cannot trick the platform into assuming the IAM role of another customer.
...
| **Trust Relationship Missing/Incorrect** | Principal is wrong, or External ID doesn't match. | STS `AssumeRole` throws `AccessDenied`. | "Access denied. Check your IAM Role permissions." | Verify the Trust Policy has the correct Principal and `ExternalId`. |
...
- **Diagnostic Steps**:
  ...
  4. Verify the `ExternalId` matches exactly (case-sensitive) the one provided by GravityLens.
